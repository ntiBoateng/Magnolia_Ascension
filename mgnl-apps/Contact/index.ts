...
export default {
  listAll: async (limit = 10): Promise<Array<ContactModel>> => {
    const baseApi = getAPIBasePath();
    const response = await axios.get(`${baseApi}/.rest/delivery/contact/?limit=${limit}`);
  ...
  list: async (query = ''): Promise<Array<ContactModel>> => {
    const baseApi = getAPIBasePath();
    const response = await axios.get(`${baseApi}/.rest/delivery/contact/?${query}`);
    const results = transformResults(response?.data?.results || []);
